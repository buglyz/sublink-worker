    ...
    <div id="workspace" x-data="formData()" x-init="init()" class="space-y-6">
      ...
      <section x-show={'$store.ui.page === "generate"'} class="mx-auto space-y-6">
        <div class="space-y-1">
          ...
          <p class="text-xs sm:text-sm text-[var(--muted-foreground)]">从节点管理中挑选节点，一键生成 Clash、Sing-Box、Surge 等订阅配置</p>
        </div>
        ...
        <form {...{ 'x-on:submit.prevent': 'submitForm' }} class="space-y-5">
          ...
          <div class="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden">
            <div class="border-b border-[var(--border)] px-5 py-4 bg-[var(--secondary)]/30">
              <div class="font-semibold text-sm text-[var(--foreground)] flex items-center gap-2">
                <i class="fas fa-layer-group text-[var(--primary)] text-xs"></i>
                <span>选择节点与规则</span>
              </div>
              <div class="text-xs text-[var(--muted-foreground)] mt-0.5">勾选参与订阅转换的节点并选择规则体系</div>
            </div>

            <div class="p-5 space-y-5">
              {/* Node Picker */}
              <div x-data="nodePickerData()" x-init="init()" class="space-y-3.5">
                <div class="flex flex-wrap items-center gap-2" x-show="$store.auth.authenticated">
                  <button
                    ...
                    x-bind:class={'allSelected ? "border-[var(--primary)] bg-[var(--primary-light)] text-[var(--primary)] font-semibold" : "border-[var(--border)] bg-[var(--card)] text-[var(--muted-foreground)] hover:text-[var(--foreground)]"'}
                  >
                  ...
                  <template x-for="p in protocols" x-bind:key="p.id">
                    <button
                      type="button"
                      class="px-2.5 py-1 rounded-lg text-xs font-medium border uppercase transition-all"
                      x-on:click="toggleProtocol(p.id)"
                      x-bind:class={'selectedProtocols[p.id] ? "border-[var(--primary)] bg-[var(--primary-light)] text-[var(--primary)] font-semibold" : "border-[var(--border)] bg-[var(--card)] text-[var(--muted-foreground)] hover:text-[var(--foreground)]"'}
                    >
                  ...
                  <div class="flex items-center gap-1.5 w-full sm:w-auto">
                    <input
                      type="search"
                      class="mm-input h-8 text-xs max-w-full sm:max-w-[12rem]"
                      placeholder="搜索节点…"
                      x-model="filter"
                    />
                    <button
                      type="button"
                      class="h-8 w-8 inline-flex items-center justify-center rounded-lg border border-[var(--border)] bg-[var(--card)] hover:bg-[var(--secondary)] text-xs text-[var(--foreground)] transition-colors shrink-0"
                      x-on:click="load()"
                ...
                <template x-if="!$store.auth.authenticated && $store.auth.authRequired">
                  <div class="text-center py-10 rounded-xl border border-dashed border-[var(--border)] bg-[var(--secondary)]/30 text-[var(--muted-foreground)] text-xs">
                    ...
                    <template x-if="enabledNodes.length">
                      <div class="space-y-2">
                        ...
                        <div class="max-h-[380px] overflow-y-auto rounded-xl border border-[var(--border)] bg-[var(--card)] divide-y divide-[var(--border)]">
                          <div class="hidden sm:grid grid-cols-[40px_1fr_100px_minmax(0,1.2fr)] gap-2 px-3 py-2 bg-[var(--secondary)]/60 text-[11px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)] sticky top-0 z-10 backdrop-blur-sm">
                          ...
                          <template x-for="n in filtered" x-bind:key="n.id">
                            <label
                              class="grid grid-cols-[40px_1fr] sm:grid-cols-[40px_1fr_100px_minmax(0,1.2fr)] gap-2 px-3 py-2 cursor-pointer hover:bg-[var(--secondary)]/50 items-center transition-colors text-xs"
                            ...
                            >
                              ...
                              <span class="hidden sm:block font-mono text-[11px] text-[var(--muted-foreground)] truncate" x-text="n.raw"></span>
                            </label>
                          </template>
                        </div>
                      </div>
              ...
              <div class="pt-3 border-t border-[var(--border)] space-y-3">
                <div class="flex items-center justify-between">
                  <label class="block text-xs font-semibold text-[var(--foreground)]">规则体系模式</label>
                ...
                <div class="space-y-3" x-show={'ruleMode === "custom"'} x-cloak>
                  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <span class="text-xs text-[var(--muted-foreground)]">{t('ruleSelection')}</span>
                    <select x-model="selectedPredefinedRule" x-on:change="applyPredefinedRule()" class="mm-select text-xs w-full sm:w-auto min-w-[10rem]">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-[260px] overflow-y-auto pr-1">
                    {UNIFIED_RULES.map((rule) => (
                      <label class="flex items-center gap-2.5 rounded-lg border border-[var(--border)] bg-[var(--card)] px-3 py-2 cursor-pointer hover:border-[var(--primary)] hover:bg-[var(--secondary)]/40 transition-all text-xs">
                ...
                <div class="space-y-4" x-show={'ruleMode === "template"'} x-cloak>
                  <div class="space-y-1">
                    <p class="text-xs text-[var(--muted-foreground)]">{t('clashTemplateHint')}</p>
                  </div>
                  <div class="space-y-2">
                    <div class="text-[11px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)]">{t('clashTemplateGroupV3')}</div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {templateGroups.v3.map((tpl) => (
                        <button
                          type="button"
                          class="rounded-lg border p-2.5 text-left transition-all hover:border-[var(--primary)] group"
                          data-tpl={tpl.id}
                        ...
                        >
                          <div class="font-semibold text-xs text-[var(--foreground)]">{tpl.label}</div>
                          <div class="font-mono text-[10px] text-[var(--muted-foreground)] mt-0.5">{tpl.id}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                  <div class="space-y-2">
                    <div class="text-[11px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)]">{t('clashTemplateGroupAethersailor')}</div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  ...
                  <div class="space-y-2">
                    <div class="text-[11px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)]">{t('clashTemplateGroupAcl4ssr')}</div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 max-h-[16rem] overflow-y-auto pr-1">
                      {templateGroups.acl4ssr.map((tpl) => (
                        <button
                          ...
                          x-bind:class={`selectedTemplate === "${tpl.id}" ? "border-[var(--primary)] bg-[var(--primary-light)] ring-1 ring-[var(--primary)]" : "border-[var(--border)] bg-[var(--card)] hover:bg-[var(--secondary)]/30"`}
                        >
                          <div class="font-semibold text-xs text-[var(--foreground)] truncate">{tpl.label}</div>
                          <div class="font-mono text-[10px] text-[var(--muted-foreground)] mt-0.5 truncate">{tpl.id}</div>
                        </button>
                      ))}
                    </div>
                  </div>
                  <p class="text-xs font-medium text-[var(--primary)]" x-show="selectedTemplate">已选模板：<span x-text="templateLabel()"></span></p>
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-2">
                <label class="flex items-center justify-between gap-2 rounded-lg border border-[var(--border)] bg-[var(--secondary)]/20 px-3 py-2 cursor-pointer hover:border-[var(--border-hover)] transition-colors">
                  <span class="text-xs font-medium text-[var(--foreground)]">{t('groupByCountry')}</span>
                ...
                <label class="flex items-center justify-between gap-2 rounded-lg border border-[var(--border)] bg-[var(--secondary)]/20 px-3 py-2 cursor-pointer hover:border-[var(--border-hover)] transition-colors">
                  <span class="text-xs font-medium text-[var(--foreground)]">{t('includeAutoSelect')}</span>
                ...
                <label class="flex items-center justify-between gap-2 rounded-lg border border-[var(--border)] bg-[var(--secondary)]/20 px-3 py-2 cursor-pointer hover:border-[var(--border-hover)] transition-colors">
                  <span class="text-xs font-medium text-[var(--foreground)]">{t('enableClashUI')}</span>
              ...
              <div class="flex flex-col sm:flex-row gap-2 pt-2">
                <button
                  type="button"
                  class="mm-btn mm-btn-primary flex-1 py-2.5 font-semibold text-sm shadow-xs"
                ...
                <button type="button" class="mm-btn mm-btn-outline px-4 text-xs" x-on:click="clearAll()">重置选择</button>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden">
            <div class="border-b border-[var(--border)] px-5 py-4 bg-[var(--secondary)]/30">
              <div class="font-semibold text-sm text-[var(--foreground)] flex items-center gap-2">
                <i class="fas fa-sliders text-[var(--primary)] text-xs"></i>
                <span>{t('customRulesSection')}</span>
              </div>
              <div class="text-xs text-[var(--muted-foreground)] mt-0.5">{t('customRulesSectionTooltip')}</div>
            </div>
            <div class="p-5">
              <CustomRules t={t} />
            </div>
          ...
          <div class="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden">
            ...
            >
              <div>
                <div class="font-semibold text-sm text-[var(--foreground)] flex items-center gap-2">
                  <i class="fas fa-gear text-[var(--primary)] text-xs"></i>
                  <span>进阶配置</span>
                </div>
                <div class="text-xs text-[var(--muted-foreground)] mt-0.5">Subconverter · Base Config · User-Agent</div>
              </div>
              <i class="fas fa-chevron-down text-xs text-[var(--muted-foreground)] transition-transform duration-200" x-bind:class={'accordionSections.advanced ? "rotate-180 text-[var(--primary)]" : ""'}></i>
            </button>
            <div x-show="accordionSections.advanced" class="border-t border-[var(--border)] p-5 space-y-5">
              <div class="space-y-2">
                <div class="text-xs font-semibold text-[var(--foreground)]">{t('subconverterConfigTitle')}</div>
                <div class="rounded-lg border border-[var(--border)] bg-[var(--secondary)]/50 px-3 py-2">
                  <p class="font-mono text-[11px] break-all text-[var(--muted-foreground)]" x-text="getSubconverterUrl()"></p>
                </div>
              ...
              <div class="space-y-2">
                <div class="text-xs font-semibold text-[var(--foreground)]">{t('baseConfig')}</div>
              ...
              <div>
                ...
                <input type="text" id="customUA" x-model="customUA" class="mm-input text-xs" placeholder="curl/7.74.0" />
              </div>
            </div>
          </div>
        </form>
        ...
        <div id="results" class="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden" x-show="generatedLinks" x-cloak>
          <div class="border-b border-[var(--border)] px-5 py-4 bg-[var(--secondary)]/30 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <div class="text-xs text-[var(--muted-foreground)] mt-0.5">复制对应客户端订阅地址</div>
            </div>
            ...
            {LINK_FIELDS.map((field) => (
              <div class="space-y-1">
                <div class="flex items-center justify-between gap-2">
                  <label class="text-xs font-semibold text-[var(--foreground)]">{t(field.labelKey)}</label>
      ...
      <section x-show={'$store.ui.page === "nodes"'} class="space-y-4">
        <div>
          ...
          <p class="text-xs sm:text-sm text-[var(--muted-foreground)] mt-1">导入、筛选与管理节点库，支持自动同步至服务端持久化保存</p>
        </div>
        <NodeLibrary t={t} />
      </section>
      ...
      <section x-show={'$store.ui.page === "subscribe"'} class="space-y-4">
        <div>
          ...
          <p class="text-xs sm:text-sm text-[var(--muted-foreground)] mt-1">快速复制节点库全量订阅或最近生成的客户端链接</p>
        </div>
        ...
        <div class="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-8 text-center space-y-3" x-show="!generatedLinks && !$store.auth.exportSubUrl">
          ...
          <div>
            <div class="font-semibold text-sm text-[var(--foreground)]">暂无可用订阅</div>
            <p class="text-xs text-[var(--muted-foreground)] mt-0.5">请先导入节点，再前往「生成订阅」或「订阅管理」创建订阅链接</p>
          </div>
          <div class="flex flex-wrap justify-center gap-2 pt-2">
            <button type="button" class="mm-btn mm-btn-outline mm-btn-sm" x-on:click={'window.__SUBLINK_UI__.setPage("nodes")'}>导入节点</button>
            <button type="button" class="mm-btn mm-btn-primary mm-btn-sm" x-on:click={'window.__SUBLINK_UI__.setPage("generate")'}>生成订阅</button>
          </div>
        </div>
        ...
        <div class="space-y-4" x-show="generatedLinks || $store.auth.exportSubUrl" x-cloak>
          ...
          <div class="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden" x-show="generatedLinks && generatedLinks.managed">
            <div class="border-b border-[var(--border)] px-5 py-4 bg-[var(--secondary)]/30">
              <div class="font-semibold text-sm text-[var(--foreground)] flex items-center gap-2">
                <i class="fas fa-bookmark text-[var(--primary)] text-xs"></i>
                <span>已保存的固定订阅</span>
              </div>
              <div class="text-xs text-[var(--muted-foreground)] mt-0.5">已写入订阅管理，可随时更新节点/模板；客户端可长期使用此链接</div>
            </div>
            <div class="p-5 space-y-3">
              ...
              <div class="flex flex-wrap gap-2">
                <button type="button" class="mm-btn mm-btn-primary mm-btn-sm text-xs" x-on:click={'const u = generatedLinks?.managed; if (!u) return; navigator.clipboard.writeText(u).then(() => alert("已复制")).catch(() => alert(u));'}>
                  <i class="fas fa-copy text-[10px]"></i> 复制订阅链接
                </button>
          ...
          <div class="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden" x-show="$store.auth.exportSubUrl">
            <div class="border-b border-[var(--border)] px-5 py-4 bg-[var(--secondary)]/30">
              <div class="font-semibold text-sm text-[var(--foreground)] flex items-center gap-2">
                <i class="fas fa-server text-[var(--primary)] text-xs"></i>
                <span>节点库全量订阅（全部启用节点）</span>
              </div>
              <div class="text-xs text-[var(--muted-foreground)] mt-0.5">自动包含节点库中所有已启用的节点，无需每次手动重新导出</div>
            </div>
            <div class="p-5 space-y-3">
              <input
                type="text"
              ...
              <div class="flex flex-wrap gap-2">
                ...
                >
                  <i class="fas fa-copy text-[10px]"></i> 复制全量订阅
                </button>
              </div>
            </div>
          </div>
          ...
          <div class="rounded-2xl border border-[var(--border)] bg-[var(--card)] shadow-sm overflow-hidden" x-show="generatedLinks">
            <div class="border-b border-[var(--border)] px-5 py-4 bg-[var(--secondary)]/30">
              <div class="font-semibold text-sm text-[var(--foreground)] flex items-center gap-2">
                <i class="fas fa-shuffle text-[var(--primary)] text-xs"></i>
                <span>多客户端转换链接</span>
              </div>
              <div class="text-xs text-[var(--muted-foreground)] mt-0.5">本次生成的 Clash / Sing-Box / Surge / Xray 转换参数链接</div>
            </div>
